...
// FetchPolicyHolders retrieves and parses policy holder data from the CSV.
func FetchPolicyHolders() ([]models.PolicyHolder, error) {
	log.Println("Fetching PolicyHolders from remote CSV...")
	...
	var policies []models.PolicyHolder
...
func SeedPolicyHolders(db *gorm.DB) {
	...
	policies, err := FetchPolicyHolders()
	if err != nil {
		log.Printf("Failed to fetch/parse CSV: %v", err)
		return
	}

	log.Printf("Seeding %d PolicyHolders...", len(policies))
